 ...
 * @note it would be nice to detect sitemaps / URL lists that we're not generated by Magento's default sitemap functionality
 * @note add other methods of outputting a status report instead of in terminal e.g. html etc.
 * @note the response could be improved
 *
 * @todo add silent check to all outputs
 * @todo change curl functions to use the Curl class
 * @todo format
 */
...
class CacheWarmerCommand extends AbstractHypernodeCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('sitemap', null, InputOption::VALUE_OPTIONAL, 'The sitemap to process, string for path or URL, int for collection choice id.', false)
            ->addOption('current-url', null, InputOption::VALUE_OPTIONAL, 'URL to check for replacement.', false)
            ->addOption('compare-url', null, InputOption::VALUE_OPTIONAL, 'The URL to replace the current url with.', false)
            ->addOption('silent', null, InputOption::VALUE_OPTIONAL, 'Silent mode, outputs all data in a associative json array.', false)
            ->addOption('format', null, InputOption::VALUE_OPTIONAL, 'Outputs the result tables in the specified format to a file, or to screen if false (default).', false)
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        // when giving a string, we cannot use data from Magento collection, so URLS are required, ask for these in next version
        if ($this->_options['sitemap'] && !is_numeric($this->_options['sitemap'])) {
            if (!$this->_options['current-url'] && !$this->_options['compare-url']) {
                $output->writeln('you have to specify the current-url and compare-url');
            } elseif ($this->_options['current-url'] && $this->_options['compare-url']) {
                $this->_options['silent'] = true;
            }
        }

        if ($input->getOption('totaltime')) {
            $this->_totalTime = true;
        }
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // setting the options again for hypernode interal usage
        $this->_options = $input->getOptions();
        ...
        if (!$this->_options['sitemap']) {
            ...
            $this->_sitemaps = $this->getSitemapFromInput($this->_options);
        }
        ...
        if ($this->_results) {
            if ($this->_options['silent']) {
                ...
                $tableData = $this->generateTablesData($input, $output, $this->_results);
                $tableHelper = $this->getHelper('table');

                foreach ($tableData as $data) {
                    $tableHelper->setHeaders($data['headers']);
                    if ($this->_options['format']) {
                        $tableHelper->renderByFormat($output, $data['requests'], $this->_options['format']); // @todo find a way to output this to a file
                    } else {
     ...
     * @return array|bool
     */
    protected function generateTablesData(InputInterface $input, OutputInterface $output, $results)
    {
    ...
    protected function prepareRequests(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($this->_sitemaps as $sitemap) {
            ...
            if (isset($sitemap['sitemap_url'])) {
                    ...
                    try {
                        ...
                        if ($curl->http_status_code == '200') {
                            try {
                                ...
                                $output->writeln('<error>' . $e->getMessage() . ' - Sitemap Metadata: ' . implode(', ', $sitemap) . '</error>');
                        ...
                        return false;
                        //@todo add error message for user - could not get sitemap from sitemap_url, maybe add url? whats in this exception
                    }
                } else {
                    $output->writeln('<error>The URL: ' . $sitemap['sitemap_url'] . ' is not valid.</error>');
                    //@todo add silent check
                }
                ...
                if (file_exists($this->_magentoRootFolder . $sitemap['relative_path'])) {
                    try {
                        ...
                        $output->writeln('<error>' . $e->getMessage() . ' - Sitemap Metadata: ' . implode(', ', $sitemap) . '</error>');
                        //@todo improve this error message + silent
                        continue;
                    }
                    ...
                    if (end(explode('.', $sitemap['relative_path'])) == 'txt') {
                        ...
                        $output->writeln('<error>Only a txt url list is currently supported for absolute paths.</error>'); //@todo silent check
                    }
                }
            }
            ...
            if ($xml) {
                ...
                // create array of urls from the sitemap XML
                foreach ($xml->children() as $child) {
                    array_push($urls, $child->loc);
                }

                /**
                 * Convention for Replacement
                 * URL not from sitemap, URL from sitemap
                 * @todo verify issue with replace strategys
                 */
                $replace = false; // equals the strategy for url modifications and comparison
                if ($this->_options['sitemap']) {
                    // the convention now is that if you specify sitemap, you have to specify both URL's "Correctly"
                ...
                foreach ($urls as $url) {
                    ...
                    if ($replace) {
                        if ($replace == 1) { // Use site from sitemap
                            ...
                            array_push($requestBatch, $this->replaceUrl($url, $this->_options['current-url'], true));
                            array_push($requestBatch, $this->replaceUrl($url, $this->_options['compare-url'], true));
                        } else {
     ...
     * @return string
     */
    public function convertTxtToXml($txt)
    {
     ...
     * @param $sitemapUrl
     * @param $replaceUrl
     * @param bool $forceInput
     * @return string
     */
    protected function replaceUrl($sitemapUrl, $replaceUrl, $forceInput = false)
    {
     ...
     * @return mixed
     */
    protected function replaceUrlByParse($sitemapUrl, $replaceUrl, $forceInput = false)
    {
     ...
     * @todo maybe show full url (without path) instead of only host
     * @param InputInterface $input
     * @param OutputInterface $output
     * @param $definedUrl
     * @param $foundUrl
     ...
     * @return mixed
     */
    public function matchUrls($url, $match)
    {
     ...
     * @todo test these input scenarios
     * @param $options
     * @return array|bool
     */
    public function getSitemapFromInput($options)
    {
        $sitemaps = array();
        if ($this->validateUrl($options['sitemap'])) {
            try {
                ...
                if ($curl->http_status_code == '200') {
                    ...
                    if (!$this->_options['silent']) {
                        throw new \RuntimeException('Fetching the sitemap did not return HTTP status 200.');
                    }
                }
            } catch (\Exception $e) {
                if (!$this->_options['silent']) {
            ...
            if ($pathType == 'relative') {
                if (file_exists($this->_magentoRootFolder . $str)) {
                    ...
                    return false;
                    // @todo output a file exist for relative sitemap error to user
                }
                ...
                if (file_exists($str)) {
                    ...
                    return false;
                    // @todo output a file exist for absolute path error to user
                }
            }
        }
        return $sitemaps;
     ...
     * @return array|bool
     */
    public function askSitemapsToProcess(InputInterface $input, OutputInterface $output)
    {
        $sitemaps = array();
     ...
     */
    public function getStoreSitemaps()
    {
        $sitemaps = array();
